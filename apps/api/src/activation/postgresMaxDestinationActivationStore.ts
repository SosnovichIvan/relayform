...
export class PostgresMaxDestinationActivationStore implements MaxDestinationActivationStore {
  ...
  async issue(ownerId: string, destinationId: string, tokenDigest: string, expiresAt: Date): Promise<void> {
    await this.database.query(
      `INSERT INTO max_destination_activations (destination_id, owner_id, token_digest, expires_at)
       VALUES ($1, $2, $3, $4)
  ...
  async consume(tokenDigest: string, recipient: string, now: Date): Promise<boolean> {
    const result = await this.database.query<{ id: string }>(
      `WITH activation AS (
         DELETE FROM max_destination_activations
         WHERE token_digest = $1 AND expires_at > $3
         RETURNING destination_id
       )
       UPDATE destinations SET recipient = $2, status = 'active'
       FROM activation
       WHERE destinations.id = activation.destination_id AND destinations.provider = 'max'
