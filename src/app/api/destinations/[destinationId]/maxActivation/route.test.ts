...
describe('MAX activation BFF', () => {
  it.each([['GET', GET], ['POST', POST]] as const)('proxies %s with an encoded destination', async (method, handler) => {
    authenticatedBackendRequest.mockResolvedValueOnce(Response.json({ status: 'pendingActivation', activationUrl: 'https://max.ru/bot?start=code' }));
    ...
    expect(authenticatedBackendRequest).toHaveBeenCalledWith('/v1/destinations/id%20%2F%20one/max-activation', { method });
  });
